               ...
               void *arg)
{
    ...
    case BLE_HS_EDONE:
        ...
        return 0;
    default:
        /* NESTOR: an error (e.g. insufficient encryption on the report map) must wake the
         * waiter too; upstream leaves esp_hidh_dev_open blocked forever. */
...
/* NESTOR: gamepads (Xbox, 8BitDo...) refuse the report map until the link is encrypted.
 * Pair/encrypt, wait for the ENC_CHANGE event, and let the caller retry the read. */
static SemaphoreHandle_t s_enc_semaphore = NULL;

static bool needs_encryption(int status)
{
    return status == BLE_HS_ERR_ATT_BASE + BLE_ATT_ERR_INSUFFICIENT_ENC ||
           status == BLE_HS_ERR_ATT_BASE + BLE_ATT_ERR_INSUFFICIENT_AUTHEN;
}

static bool ensure_encrypted(uint16_t conn_handle)
{
    if (!s_enc_semaphore) s_enc_semaphore = xSemaphoreCreateBinary();
    xSemaphoreTake(s_enc_semaphore, 0);
    int rc = ble_gap_security_initiate(conn_handle);
    ESP_LOGI(TAG, "pairing for encrypted link (rc=%d)", rc);
    if (rc != 0 && rc != BLE_HS_EALREADY) return false;
    return xSemaphoreTake(s_enc_semaphore, pdMS_TO_TICKS(15000)) == pdTRUE;
}
...
static int read_char(uint16_t conn_handle, uint16_t handle, uint8_t **out, uint16_t *out_len)
{
    ...
    WAIT_CB();
    if (needs_encryption(s_read_status) && ensure_encrypted(conn_handle)) {
        s_read_data_val = NULL;
        s_read_data_len = 0;
        rc = ble_gattc_read_long(conn_handle, handle, 0, nimble_on_read, NULL);
        if (rc != 0) return rc;
...
esp_hidh_gattc_event_handler(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_ENC_CHANGE:
        ...
        if (event->enc_change.status == 0 && s_enc_semaphore) xSemaphoreGive(s_enc_semaphore);
        return 0;
    case BLE_GAP_EVENT_REPEAT_PAIRING: {
        /* peer lost its bond: drop ours and pair again */
        struct ble_gap_conn_desc d;
        if (ble_gap_conn_find(event->repeat_pairing.conn_handle, &d) == 0) ble_store_util_delete_peer(&d.peer_id_addr);
